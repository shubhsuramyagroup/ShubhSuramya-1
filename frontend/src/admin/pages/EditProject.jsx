...
import { getSingleProject, updateProject } from "../../services/projectService";
  ...
  return (
    <div style={{ marginTop: "12px" }}>
      ...
      <div
        ...
        onMouseEnter={(e) => {
          if (!uploading) e.currentTarget.style.borderColor = "#E4572E";
        }}
        onMouseLeave={(e) => {
...
export default function EditProject() {
  ...
  useEffect(() => {
    const fetchProject = async () => {
      try {
        ...
        // Ensure all array fields have at least one empty item
        setProjectData({
          ...data,
          youtubeVideos: data.youtubeVideos?.length ? data.youtubeVideos : [""],
          stats: data.stats?.length ? data.stats : [{ title: "", value: "" }],
          ...
          floors: data.floors?.length
            ? data.floors
            : [{ floorName: "", rooms: [{ name: "", image: "" }] }],
          projectTags: data.projectTags?.length ? data.projectTags : [""],
        });
        setLoading(false);
      } catch (error) {
        console.error(error);
  ...
  const handleYoutubeChange = (i, val) => {
    const u = [...projectData.youtubeVideos];
    u[i] = val;
  ...
  const handleTagChange = (i, val) => {
    const updated = [...projectData.projectTags];
  ...
  const removeTag = (i) => {
    ...
    setProjectData({
      ...projectData,
      projectTags: updated.length ? updated : [""],
    });
  };

  // Stats
  const handleStatsChange = (i, field, val) => {
    const u = [...projectData.stats];
    u[i][field] = val;
  ...
  const removeStat = (i) => {
    ...
    setProjectData({
      ...projectData,
      stats: u.length ? u : [{ title: "", value: "" }],
    });
  };

  // Amenities
  const handleAmenityChange = (i, field, val) => {
    const u = [...projectData.amenities];
    u[i][field] = val;
  ...
  const removeAmenity = (i) => {
    ...
    setProjectData({
      ...projectData,
      amenities: u.length ? u : [{ title: "", image: "" }],
    });
  };

  // Gallery
  const handleGalleryChange = (i, val) => {
    const u = [...projectData.galleryImages];
    u[i] = val;
  ...
  const handleNearbyChange = (i, field, val) => {
    const u = [...projectData.nearbyPlaces];
    u[i][field] = val;
  ...
  const removeNearby = (i) => {
    ...
    setProjectData({
      ...projectData,
      nearbyPlaces: u.length ? u : [{ name: "", type: "", distance: "" }],
    });
  };
    ...
    <div style={{ background: "#F8F7F4", minHeight: "100vh", padding: "40px" }}>
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        ...
        >
          ...
          <h1 style={{ fontSize: "42px", color: "#1F2A44", margin: 0 }}>
            Edit Project
          </h1>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={sectionStyle}>
            ...
            <input
              ...
              type="text"
              name="projectHeading"
              value={projectData.projectHeading}
            ...
            >
              ...
              <input
                ...
                placeholder="Units (e.g. 120 Units)"
                onChange={handleChange}
              />
              <input
                style={{ ...inputStyle, marginTop: 0 }}
                ...
                value={projectData.possessionTiming}
                placeholder="Possession Timing (e.g. Dec 2026)"
                onChange={handleChange}
              />
              <input
                style={{ ...inputStyle, marginTop: 0 }}
                ...
                placeholder="Starting Price (e.g. ₹45 Lakh)"
                onChange={handleChange}
              />
            </div>
          </div>
          ...
          <div style={sectionStyle}>
            ...
            >
              Example: Luxury, Smart Home, River View, 3BHK, Premium Living
            </p>
          ...
          <div style={sectionStyle}>
            ...
            <p style={{ fontSize: "13px", color: "#888", margin: "0 0 4px" }}>
              This image will be uploaded to Cloudinary. Only the URL is saved
              to Firebase.
            </p>
            <ImageUploadField
              ...
              onChange={(url) =>
                setProjectData({ ...projectData, mainImage: url })
              }
          ...
          <div style={sectionStyle}>
              ...
              >
                ...
                >
                  <input
                    style={{ ...inputStyle, marginTop: 0 }}
                    type="text"
                    placeholder="Stat Title (e.g. Total Floors)"
                    value={stat.title}
                    onChange={(e) =>
                      handleStatsChange(i, "title", e.target.value)
                    }
                  />
                  <input
                    style={{ ...inputStyle, marginTop: 0 }}
                    type="text"
                    placeholder="Stat Value (e.g. 22)"
                    value={stat.value}
                    onChange={(e) =>
                      handleStatsChange(i, "value", e.target.value)
                    }
                  />
                </div>
              </div>
            ...
            <button
              ...
              onClick={() =>
                setProjectData({
                  ...projectData,
                  stats: [...projectData.stats, { title: "", value: "" }],
                })
              }
            >
              + Add Stat
            ...
            {projectData.amenities.map((amenity, i) => (
              ...
              >
                ...
                <input
                  ...
                  value={amenity.title}
                  onChange={(e) =>
                    handleAmenityChange(i, "title", e.target.value)
                  }
                />
                <ImageUploadField
          ...
          <div style={sectionStyle}>
            ...
            <p style={{ fontSize: "13px", color: "#888", margin: "0 0 4px" }}>
              Paste your PDF links directly (Google Drive, Cloudinary raw
              upload, etc.)
            </p>
            <input
              style={inputStyle}
            ...
            <input
              ...
              type="text"
              name="floorPlanPdf"
              value={projectData.floorPlanPdf}
              placeholder="Floor Plan PDF URL"
              onChange={handleChange}
            />
          </div>
          ...
          <div style={sectionStyle}>
            <h2>Floor Details</h2>
            <p style={{ fontSize: "13px", color: "#888", margin: "0 0 4px" }}>
              Add each floor with its rooms (e.g. Kitchen, Master Bedroom,
              Drawing Room) and room images.
            </p>
                ...
                {floor.rooms.map((room, ri) => (
                  ...
                  >
                    ...
                    <input
                      ...
                      value={room.name}
                      onChange={(e) =>
                        handleRoomChange(fi, ri, "name", e.target.value)
                      }
                    />
                    <ImageUploadField
                      label="Room Image"
              ...
              >
                ...
                >
                  <input
                    style={{ ...inputStyle, marginTop: 0 }}
                    type="text"
                    placeholder="Name (e.g. Ayush Hospital)"
                    value={place.name}
                    onChange={(e) =>
                      handleNearbyChange(i, "name", e.target.value)
                    }
                  />
                  <input
                    style={{ ...inputStyle, marginTop: 0 }}
                    ...
                    value={place.type}
                    onChange={(e) =>
                      handleNearbyChange(i, "type", e.target.value)
                    }
                  />
                  <input
                    style={{ ...inputStyle, marginTop: 0 }}
                    ...
                    onChange={(e) =>
                      handleNearbyChange(i, "distance", e.target.value)
                    }
                  />
                </div>
              </div>
