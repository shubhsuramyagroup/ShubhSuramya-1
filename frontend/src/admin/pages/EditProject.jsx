...
export default function EditProject() {
    ...
    const fetchProject = async () => {
      try {
        ...
        setProjectData({
          ...
          amenities: data.amenities?.length ? data.amenities : [{ title: "", image: "" }],
          galleryImages: data.galleryImages?.length ? data.galleryImages : [""],
          nearbyPlaces: data.nearbyPlaces?.length ? data.nearbyPlaces : [{ name: "", type: "", distance: "" }],
          floors: data.floors?.length ? data.floors : [{ floorName: "", rooms: [{ name: "", image: "" }] }],
          projectTags: data.projectTags?.length ? data.projectTags : [""],
        });
        setLoading(false);
      } catch (error) {
        console.error(error);
        setToast({ type: "error", message: "Failed to load project data." });
        setLoading(false);
      }
    };
    fetchProject();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => navigate("/admin/projects"), 1800);
    } catch (error) {
      console.error(error);
      setToast({ type: "error", message: error.message || "Failed to update project." });
    } finally {
      setSaving(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F8F7F4] flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-[#1F2A44] font-semibold text-sm">Loading project…</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#F8F7F4]">
      ...
      <div className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-gray-100
        ...
        <span className="text-[#1F2A44] font-semibold text-sm truncate max-w-[200px] sm:max-w-none">
          {projectData.title || "Edit Project"}
        </span>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-8 py-8 sm:py-12">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold text-[#1F2A44]">Edit Project</h1>
          <p className="text-gray-400 text-sm mt-1">Update the details below and save your changes</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input type="text" name="title" value={projectData.title}
                  placeholder="e.g. Skyline Heights" onChange={handleChange} />
              </div>
              <div>
                <FieldLabel>Description</FieldLabel>
                <Textarea name="description" value={projectData.description}
                  placeholder="Project Description" onChange={handleChange} />
              </div>
              <div>
                <FieldLabel>Project Heading</FieldLabel>
                <Input type="text" name="projectHeading" value={projectData.projectHeading}
                  placeholder="Project Heading" onChange={handleChange} />
              </div>
          ...
          <Section title="Project Tags">
            <p className="text-[11px] text-gray-400 mb-3">Example: Luxury, Smart Home, River View, 3BHK, Premium Living</p>
          ...
          <Section title="Project Documents">
            ...
            <div className="space-y-3">
              <div>
                ...
                <Input type="text" name="brochurePdf" value={projectData.brochurePdf || ""}
                  placeholder="Brochure PDF URL" onChange={handleChange} />
              </div>
              <div>
                <FieldLabel>Floor Plan PDF URL</FieldLabel>
                <Input type="text" name="floorPlanPdf" value={projectData.floorPlanPdf || ""}
                  placeholder="Floor Plan PDF URL" onChange={handleChange} />
              </div>
            </div>
          </Section>
